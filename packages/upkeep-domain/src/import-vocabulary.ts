...
const LANGUAGE_BY_LABEL = new Map<string, string>(
  LANGUAGE_CODES.map((code) => [normalise(LANGUAGE_LABELS[code]), code]),
);
const KNOWN_LANGUAGE_CODES = new Set<string>(LANGUAGE_CODES);
...
export function parseLanguage(value: string | null | undefined): string | null {
  ...
  if (KNOWN_LANGUAGE_CODES.has(key)) return key;
