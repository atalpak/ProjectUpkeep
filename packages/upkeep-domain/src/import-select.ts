...
/**
 * A parsed row with its printing (or the reason there is none). Defined here,
 * not in resolve.ts, because the pure planners need the shape and resolve.ts
 * is server-only; resolve.ts re-exports it.
 */
...
export function choosePrinting(
  ...
  // inSet is non-empty here (the empty case returned above); the assertion only
  // satisfies this package's stricter noUncheckedIndexedAccess.
