...
export function CardInspector() {
  ...
  if (!inspect.big) {
    ...
    return <div aria-hidden="true" className={`pointer-events-none fixed right-3 top-16 z-[6400] w-56 ${settings.cardDetails ? "xl:hidden" : ""}`}><CardArt src={card.imageNormal ?? card.imageSmall} alt={card.name} label={card.name} className="shadow-2xl" /></div>;
  }
