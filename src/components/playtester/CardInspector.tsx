...
export function CardInspector() {
  ...
  if (!inspect.big) {
    ...
    return <div aria-hidden="true" className={`pointer-events-none fixed right-3 top-3 z-[6400] w-64 ${settings.cardDetails ? "xl:hidden" : ""}`}><CardArt src={card.imageNormal ?? card.imageSmall} alt={card.name} label={card.name} className="shadow-2xl" /></div>;
  }
