...
export function ZoneBrowser({ onClose }: { onClose: () => void }) {
  const game = useGame(); const store = usePlayStore(); const env = usePlayEnv(); const request = useUi((s) => s.zone);
  ...
  const close = () => { if (zone === "library" && request.mode === "search" && env.settings.get().shuffleOnClose) store.dispatch({ type: "SHUFFLE", zone: "library", seed: crypto.getRandomValues(new Uint32Array(1))[0] }); onClose(); };
